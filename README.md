...
- 支持用一个主框架统领复合文档，并在局部章节使用教程、复盘或方案等辅助框架；
- 检查相邻章节的时序、因果、递进、对比或行动承接，避免只靠过渡词拼接内容；
- 一个段落只承载一个主要意思；
- 简单问题不用标题，列表和表格只在降低理解成本时使用；
- 长内容按背景、解释、实现或证据、影响或下一步逐层展开；
- 正式文档默认保留有用的编号、缩进、代码块、表格和导航结构；
- 操作教程补齐前置条件、执行上下文、预期结果、验证方法和失败恢复；
...
`SKILL.md` 保存核心流程；`references/formats.md` 提供文档结构模板；`references/patterns.md` 是 AI 写作模式复查清单；`references/review-checklist.md` 用于结构、表达和交付前检查；`evals/cases.yaml` 覆盖短问答、复合文档、问答沉淀、格式保真、操作教程、技术文本和对外文案等评测场景。
